...
```bash
MARKOVIAN_IMPL=/abs/path/to/your/index.ts npx tsx runner.ts
```

On Windows, use forward slashes and an absolute path. The module must export a `Markovian` class plus `modelTotals` and `runEfficiency`.
